...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    // Enviar la siguiente letra del alfabeto
uint8_t tx_char = rx_data;
if ((rx_data >= 'A' && rx_data < 'Z') || (rx_data >= 'a' && rx_data < 'z'))
{
  tx_char = rx_data + 1;
}
else if (rx_data == 'Z') tx_char = 'A';
else if (rx_data == 'z') tx_char = 'a';
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 250) // Cada 250 ms
    {
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      sprintf(tx_buffer, "Boton B1 detectado: LED EXT ON por 5s\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
